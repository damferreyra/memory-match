...
export class MenuScene extends Phaser.Scene {
	...
	create(): void {
		...
		const drawBtn = (color: number) => {
			btn.clear();
			btn.fillStyle(color);
			btn.fillRoundedRect(btnX - btnW / 2, btnY - btnH / 2, btnW, btnH, 16);
		};
		drawBtn(CTA_PRIMARY_COLOR);
		...
		zone.on('pointerover', () => drawBtn(CTA_PRIMARY_COLOR_HOVER));
		zone.on('pointerout', () => drawBtn(CTA_PRIMARY_COLOR));
		zone.on('pointerdown', () => {
